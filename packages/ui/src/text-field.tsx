...
export const TextField = React.forwardRef<TextFieldAPI, TextFieldProps>(
  function TextField(
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (onFormat) newValue = onFormat(newValue)

      // notify parent first
      onChange?.(e)

      // then update internal state (possibly formatted)
      ...
      if (status === 'success') setStatus(undefined)
    ...
    const handleVerify = async () => {
      const isVerified = await onVerify?.(inputValue as string)
