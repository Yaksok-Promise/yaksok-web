...
const TestComponent = ({
  steps,
  defaultStep,
}: { steps: string[]; defaultStep: string }) => {
  const { Funnel, Step, currentStep, handleNext } = useFunnel<string>(
    steps,
    defaultStep
  )
  ...
  return (
    <div>
      ...
      <button onClick={handleNext}>Go to Step 2</button>
      <Funnel>
        <Step name="step1">
...
describe('useFunnel', () => {
  ...
  it('초기 step이 올바르게 설정되어야 한다', () => {
    render(<TestComponent defaultStep="step1" steps={['step1', 'step2']} />)
